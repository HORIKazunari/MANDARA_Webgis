 ...
 * @returns
 */
var rectangle = function (left_point, right_size, top, bottom) {
 ...
 * @returns
 */
var PeripheriDirinfo = function (Code, Dir) {
    this.Code = Code;
    this.Dir = Dir;
};
/**
...
strData_info.prototype.Clone = function (NoValueFlag = false) {
    ...
    var dts = dt.SoloModeViewSettings;
    // @ts-expect-error TS(2304): Cannot find name 'dts'.
 ...
 * @returns
 */
var strGraph_Data_Oresen = function () {
    this.Size; //Single
    // @ts-expect-error TS(7009): 'new' expression, whose target lacks a construct s... Remove this comment to see the full error message
 ...
 * @returns
 */
var strSeries_DataSet_Item_Info = function () {
    this.Layer;
    this.Data;
...
var clsAttrData = function () {
                ...
                switch (lay.Print_Mode_Layer) {
                    case enmLayerMode_Number.SoloMode: {
                        if (this.Get_DataNum(Layernum) == 0) {
                            ...
                            if ((md == enmSoloMode_Number.ContourMode) && ((laydata.SoloModeViewSettings.ContourMD.Interval_Mode == enmContourIntervalMode.ClassPaint) || (laydata.SoloModeViewSettings.ContourMD.Interval_Mode == enmContourIntervalMode.ClassHatch))) {
    ...
    this.Sort_OverLay_Data_Sub = function (Ov_Data) {
        ...
        PicUpShape.push(enmShape.PolygonShape);
        PicUpMode.push("0");
        PicUpShape.push(enmShape.PolygonShape);
        PicUpMode.push("4");
        PicUpShape.push(enmShape.PolygonShape);
        PicUpMode.push("3");
        PicUpShape.push(enmShape.LineShape);
        PicUpMode.push("016");
        PicUpShape.push(enmShape.PolygonShape);
        PicUpMode.push("125678");
        PicUpShape.push(enmShape.PolygonShape);
        PicUpMode.push("9A");
        PicUpShape.push(enmShape.PolygonShape);
        PicUpMode.push("B");
        PicUpShape.push(enmShape.LineShape);
        PicUpMode.push("2349");
        PicUpShape.push(enmShape.PointShape);
        PicUpMode.push("3");
        PicUpShape.push(enmShape.PointShape);
        PicUpMode.push("0");
        PicUpShape.push(enmShape.PointShape);
        PicUpMode.push("1245678");
        PicUpShape.push(enmShape.PointShape);
        PicUpMode.push("9A");
        PicUpShape.push(enmShape.PointShape);
        PicUpMode.push("B");
    ...
    this.Set_Legend = function (D_Layer, D_DataNum, O_Data, ClassPaintF, MarkSizeF, MarkSizeValueCopyF, MarkBlockF, ContourF, ClassMarkF, ClassODF, StringModeF, MarkBarF, ClassODOriginCopyF, copyMarkCommonInnerDataF) {
    ...
    this.Change_Acc_Position_by_Accessory_Base_Set_Screen = function () {
        ...
        let H = ms.height();
        // @ts-expect-error TS(7009): 'new' expression, whose target lacks a construct s... Remove this comment to see the full error message
        mv.MapTitle.Position = new point(Generic.m_min_max((mv.MapTitle.Position.x - lft) / w, 0.1, 0.9), Generic.m_min_max((mv.MapTitle.Position.y - tp) / H, 0.1, 0.95));
        // @ts-expect-error TS(7009): 'new' expression, whose target lacks a construct s... Remove this comment to see the full error message
        mv.MapScale.Position = new point(Generic.m_min_max((mv.MapScale.Position.x - lft) / w, 0.1, 0.8), Generic.m_min_max((mv.MapScale.Position.y - tp) / H, 0.1, 0.95));
        // @ts-expect-error TS(7009): 'new' expression, whose target lacks a construct s... Remove this comment to see the full error message
        mv.DataNote.Position = new point(Generic.m_min_max((mv.DataNote.Position.x - lft) / w, 0.1, 0.9), Generic.m_min_max((mv.DataNote.Position.y - tp) / H, 0.1, 0.95));
        // @ts-expect-error TS(7009): 'new' expression, whose target lacks a construct s... Remove this comment to see the full error message
        mv.AttMapCompass.Position = new point(Generic.m_min_max((mv.AttMapCompass.Position.x - lft) / w, 0.1, 0.9), Generic.m_min_max((mv.AttMapCompass.Position.y - tp) / H, 0.1, 0.9));
        for (let i = 0; i < mv.MapLegend.Base.Legend_Num; i++) {
            // @ts-expect-error TS(7009): 'new' expression, whose target lacks a construct s... Remove this comment to see the full error message
            mv.MapLegend.Base.LegendXY[i] = new point(Generic.m_min_max((mv.MapLegend.Base.LegendXY[i].x - lft) / w, 0.1, 0.8), Generic.m_min_max((mv.MapLegend.Base.LegendXY[i].y - tp) / H, 0.1, 0.8));
        }
    };
    //変換した座標を計算済み座標に登録
    ...
    this.SetDataFromMDRJ = function (MapDataList, attrText) {
        ...
        if (olp.Lat_Interval == undefined) {
            olp.Lat_Interval = 1;
        }
        if (olp.Lon_Interval == undefined) {
            olp.Lon_Interval = 1;
        }
    ...
    this.Set_Data_from_String = function (LayerReading, TotalMissing) {
        ...
        this.Add_one_Layer(LayerReading.Name, LayerReading.Type, LayerReading.MeshType, LayerReading.Shape, LayerReading.MapFile, LayerReading.Time, LayerReading.ReferenceSystem, LayerReading.Comment_Temp, Object_num, Get_Obj);
    ...
    this.SetMapViewerData = function (MapDataList, LayDataInf, DeleteDefDataFlag) {
        ...
        for (let key in LayDataInf) {
            ...
            this.Add_one_Layer(layData.Name, enmLayerType.Normal, enmMesh_Number.mhNonMesh, layshape, layData.MapfileName, layData.Time, enmZahyo_System_Info.Zahyo_System_No, "", objn, Get_Obj);
                                ...
                                case enmAttDataType.Strings: {
                                    ...
                                    switch (tdcc.Condition) {
                                        ...
                                        case enmCondition.Head:
                                            if ((V.left(tdcc.Val.length)) == tdcc.Val) {
                                                f = true;
                                            }
                                            break;
                                        case enmCondition.Foot:
                                            if (V.right(tdcc.Val.length) == tdcc.Val) {
                                                f = true;
                                            }
                                            break;
                                    }
