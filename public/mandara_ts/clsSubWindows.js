...
function clsColorPicker(event_point, okEvent) {
    ...
    for (var i = 0; i <= 55; i++) {
        ...
        colorDivObj.style.position = 'absolute';
        colorDivObj.style.width = colPSize.picW.px();
        colorDivObj.style.height = colPSize.picH.px();
        colorDivObj.style.left = Math.floor(colPSize.picMargin + Math.floor(i / 7) * (colPSize.picW + colPSize.picInterval)).px();
        colorDivObj.style.top = Math.floor((i % 7) * (colPSize.picH + colPSize.picInterval) + colPSize.picMargin + scrMargin.top).px();
        colorDivObj.style.border = "1px solid #888888";
        colorDivObj.style.backgroundColor = colorList[i];
    ...
    rangeNameObj.style.position = 'absolute';
    rangeNameObj.style.fontSize = "12px";
    rangeNameObj.style.color = "#000000";
    rangeNameObj.style.top = (colPSize.rangeTop).px();
    rangeNameObj.style.left = (50).px();
    ...
    rangeObj.style.width = colPSize.rangeWidth.px();
    rangeObj.style.top = (colPSize.rangeTop).px();
    rangeObj.style.left = (100).px();
    ...
    colorPickerBox.style.position = 'absolute';
    colorPickerBox.style.top = (colPSize.buttonTop - 3).px();
    colorPickerBox.style.left = (colPSize.picMargin * 2).px();
    colorPickerBox.style.width = "60px";
    colorPickerBox.style.height = (colPSize.buttonHeight).px();
    colorPickerBox.style.backgroundColor = ocol.toRGBA();
    colorPickerBox.style.opacity = ocol.a;
    colorPickerBox.style.border = "1px solid #444444";
...
// @ts-expect-error TS(7006): Parameter 'event' implicitly has an 'any' type.
function clsSelectData(event, _attrData, Layernum, okEvent, PreAstarisk = undefined, Number_Print_F = true, Normal_F = true, Category_f = true, String_f = true) {
...
function frmPrintOption(firstTab = 0) {
    ...
    Generic.createNewButton(tab00, "方位設定", "", 30, 113, function () {
        // @ts-expect-error TS(7006): Parameter 'v' implicitly has an 'any' type.
...
function frmMainCopyDataSettings(okEvent) {
    ...
    function buttonOK() {
        ...
        for (let i in checked) {
            let n = checked[i];
            // @ts-expect-error TS(2304): Cannot find name 'attrData'.
            attrData.Set_Legend(Dlyaer, n, attrData.LayerData[OLayer].atrData.Data[OData], ClassCopyF, MarkCopyF, MarkSizeValueCopyF, MarkCopyF, ContourCopyF, ClassCopyF, ClassCopyF, MarkCopyF, MarkCopyF, ODOriginCopyF, (Dlyaer == OLayer));
        }
        okEvent();
    }
}
...
function frmMain_SetSeriesMode(okEvent) {
    ...
    // @ts-expect-error TS(7009): 'new' expression, whose target lacks a construct s... Remove this comment to see the full error message
    let seriesListView = new ListViewTable(gbdestSeries, "", "", "", seriesHdata, [], 10, 110, 300, 170, slborderStyle, "font-size:13px;", "background-Color:#dddddd;text-align:center", "", ["width:10%"], ["text-align:center"], true, undefined);
...
function frmMain_ConditionSettingSub(_ConItem, okEvent) {
    ...
    // @ts-expect-error TS(7009): 'new' expression, whose target lacks a construct s... Remove this comment to see the full error message
    let ListView = new ListViewTable(grPanel, "", "", "", overHdata, [], 10, 10, 290, 165, borderStyle, "font-size:13px;", "background-Color:#dddddd;text-align:center", "", ["", "", "width:30%"], "", true, undefined);
...
function frmMain_Buffer(okEvent) {
    ...
    let txtBuffer = Generic.createNewWordNumberInput(gbMethod, "バッファ距離", "", "", "", 40, 130, undefined, 100, function () {
...
function frmPrint_DummyObjectGroup() {
    ...
    Generic.createNewButton(gbDummyList, "削除", "", 115, 150, function () {
        // @ts-expect-error TS(2304): Cannot find name 'lstDummyItem'.
    ...
    // @ts-expect-error TS(7006): Parameter 'e' implicitly has an 'any' type.
    function txtKeyDown(e) { if (e.keyCode == 13) {
        btnAdd.click();
    } }
    ...
    const chkDummyClip = Generic.createNewCheckBox(gbDummyFrame, "面形状ダミーオブジェクトをレイヤのクリッピング領域に設定", "", PolygonDummy_ClipSet_F[LayerNum], 215, 260, 150, function () {
