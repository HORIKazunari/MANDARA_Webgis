            ...
            if (aodd.Print_Mode_Layer == enmLayerMode_Number.SoloMode) {
                ...
                for (let j = 0; j < Num; j++) {
                    if (i == j) {
                        ...
                        if ((ovDataj.Print_Mode_Layer == enmLayerMode_Number.SoloMode) && (ovDataj.Layer == aodd.Layer) && (ovDataj.Mode == aodd.Mode)) {
                            Equal_Mode_N++;
                        }
                    }
                }
            ...
            if (d2[i].Print_Mode_Layer == enmLayerMode_Number.SoloMode) {
                ...
                for (let j = 0; j < d2.length; j++) {
                    if (i == j) {
                        ...
                        let ovDataj = d2[j];
                        if ((ovDataj.Print_Mode_Layer == enmLayerMode_Number.SoloMode) && (ovDataj.Layer == d2[i].Layer) && (ovDataj.Mode == d2[i].Mode)) {
                            Equal_Mode_N++;
                        }
                    }
                }
        ...
        if (C_md.Interval_Mode == enmContourIntervalMode.ClassPaint) {
            ...
            for (let i = 0; i <= hn; i++) {
                if ((HnPolygon[i].fnum > 0) || (HnPolygon[i].CNum > 0)) {
                    this.ContourPolyBoundary(g, LayerNum, DataNum, i, hn, C_md.Interval_Mode, HnPolygon[i], Pre_CStac, Frame_AllPoint, C_md.Spline_flag, ST);
                }
            }
        }
        for (let i = 0; i < ln; i++) {
 ...
 * @param {*} SplineT
 */
    static ContourPolyBoundary(g, Layernum, DataNum, Pcon, hn, Interval_Mode, HnPolygon, Pre_CStac, Frame_AllPoint, Spline_flag, SplineT) {
        let spxy = [];
        let epxy = [];
 ...
 */
    static ContourMesh_Value(Layernum, DataNum, DataValue, P, md, F_Mesh, F_Mesh_In) {
        ...
        if ((a - (fa - 360) < alimit) && (nn >= 2) && (a - (ffa - 360) < alimit) && (Math.abs(O_Distance[AngleSort.DataPosition(fai)] - O_Distance[AngleSort.DataPosition(OI)]) > 0.1)) {
        ...
        for (let i = sti; i < ad.SoloModeViewSettings.Div_Num; i++) {
            ...
            for (let j = 0; j < objN; j++) {
                if (Category_Array[j] == i) {
                    // @ts-expect-error TS(2304): Cannot find name 'attrData'.
                    if ((attrData.Check_Condition(Layernum, j) == true) && ((Category_Array[j] != -1) || (av.Missing_Data.Print_Flag == true))) {
                        // @ts-expect-error TS(2304): Cannot find name 'attrData'.
