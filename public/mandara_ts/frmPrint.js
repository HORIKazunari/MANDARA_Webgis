                        ...
                        switch (attrData.TempData.frmPrint_Temp.PrintMouseMode) {
                            case enmPrintMouseMode.Normal: {
                                ...
                                if (retV.type == Check_Acc_Result.NoAccessory) {
                                    ...
                                    //非表示の飾りを表示させるメニューの表示
                                    if ((av.MapTitle.Visible == false) || (av.MapLegend.Base.Visible == false) || (av.MapScale.Visible == false) || (av.AttMapCompass.Visible == false) || (av.AttMapCompass.Visible == false) || (av.AccessoryGroupBox.Visible == false)) {
                                    ...
                                    function accVisible(data, e) {
                                        switch (data.caption) {
                                            ...
                                            case "方位設定":
                                                // @ts-expect-error TS(2304): Cannot find name 'attrData'.
                                                frmCompassSettings(attrData.TotalData.ViewStyle.AttMapCompass, function (v) {
                                                    // @ts-expect-error TS(2304): Cannot find name 'attrData'.
                            ...
                            mnuAccPopupVisible.push({
                                caption: "記号表示位置移動", event: function () {
                                    ...
                                    if (attrData.TempData.frmPrint_Temp.SymbolPointFirstMessage == true) {
                                        if (window.confirm("新しい記号表示位置をクリックして指定します(右クリックでキャンセル)。") == false) {
                                            return;
                                        }
                                    }
                                    // @ts-expect-error TS(2304): Cannot find name 'attrData'.
                            ...
                            mnuAccPopupVisible.push({
                                caption: "ラベル表示位置移動", event: function () {
                                    ...
                                    if (attrData.TempData.frmPrint_Temp.LabelPointFirstMessage == true) {
                                        if (window.confirm("新しいラベル表示位置をクリックして指定します(右クリックでキャンセル)。") == false) {
                                            return;
                                        }
                                    }
                                    // @ts-expect-error TS(2304): Cannot find name 'attrData'.
    ...
    function Near_OD(MapP) {
        ...
        switch (attrData.TotalData.LV1.Print_Mode_Total) {
            case enmTotalMode_Number.DataViewMode: {
                ...
                if (al.Print_Mode_Layer == enmLayerMode_Number.SoloMode) {
                    // @ts-expect-error TS(2304): Cannot find name 'attrData'.
                    if ((attrData.getSoloMode(Layernum, DataNum) == enmSoloMode_Number.ClassODMode) && (al.Shape != enmShape.LineShape) && (al.Type != enmLayerType.Trip)) {
        ...
        function Near_OD_sub(MapP, Layernum, DataNum) {
            ...
            for (let i = 0; i < objn; i++) {
                ...
                if (Cate != -1) {
                    let D;
                    if (((i == mod.O_object) && (Layernum == mod.o_Layer)) || ((al.atrData.Data[DataNum].SoloModeViewSettings.Class_Div[Cate].ODLinePat.BlankF == false) && (
                    // @ts-expect-error TS(2304): Cannot find name 'attrData'.
