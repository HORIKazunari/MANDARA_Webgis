...
- `index.html` のサンプル起動リンクは `mandarawebgis.html#preset=...` を使います
- 旧 `?file=` による任意 URL 直接読み込みは廃止しています
- `.mdrj` / `.mdrmj` 読み込み時に参照された地図ファイルが `map/` に存在する場合は、必要時に自動取得します
- サーバー上に対象 `.mpfj` が存在しない場合は、利用者が地図ファイル追加またはドラッグ&ドロップで読み込む必要があります
- `.mdrmj` は地図データを内包できるため、外部地図ファイルなしで開ける場合があります

運用上の前提:

- 公開サーバーには `dist/data` に加えて `dist/map` も配置してください
- 初期ロードサイズ抑制のため、JAPAN / WORLD / 日本緯度経度の地図は起動時に先読みしません
