...
type SelectControl = HTMLSelectElement & {
    getValue: () => string;
    getText: () => string;
    addSelectList: (items: Array<{ value: string | number; text: string }>, selectedIndex?: number, replace?: boolean, allowBlank?: boolean, blankValue?: number) => void;
...
export function clsMarkSet(event: MouseEvent, okEvent: (mark: Mark) => void, mark: Mark, _attrData: IAttrData): void {
    ...
    clsDrawTile.Darw_Sample_BackGroundBox(backrColorBox, mk.WordFont.Back, _attrData.TotalData.ViewStyle.ScrData as unknown as Screen_info);
...
export function frmPrint_ObjectValue(_attrData: IAttrData, okEvent: () => void) {
    ...
    const valueShow = _attrData.TotalData.ViewStyle.ValueShow as {
        Clone?: () => unknown;
    } & typeof _attrData.TotalData.ViewStyle.ValueShow;
    const avs = (typeof valueShow.Clone === 'function' ? valueShow.Clone() : valueShow) as typeof _attrData.TotalData.ViewStyle.ValueShow;
...
export function frmPrint_backImageSet(_attrData: IAttrData, okEvent: () => void) {
    ...
    const currentTileMap = avt.TileMapDataSet as { opt: { tag: string; id: string } };
    const finedx = tag.indexOf(currentTileMap.opt.tag);
    ...
    setTileMapListByTag(currentTileMap.opt.id);
...
export function graphModeEn_Obi() {
    ...
    const nowGraph = nowGraphRaw as { En_Obi: GraphEnObiControl; GraphMode: number };
    const En_Obi = nowGraph.En_Obi;
    const graphMode = Number(nowGraph.GraphMode);
...
function frmLatLonInput(LatLon: latlon, BoxF: boolean, okEvent: (LatLon: latlon) => void) {
    ...
    function buttonOK(){
        if (appState().settingData.Ido_Kedo_Print_Pattern === enmLatLonPrintPattern.DegreeMinuteSecond) {
            const dms = new strLatLonDegreeMinuteSecond() as IStrLatLonDegreeMinuteSecond;
...
export function frmPrintOption(firstTab: number = 0) {
    ...
    const atvaBackCanvas = Generic.createNewWordDivCanvas(tab01, "atvaBack", "背景", 10, 180,undefined, function(e: MouseEvent){
        ...
        function backGet(back: BackGround_Box_Property) {
            atva.Back = back;
            clsDrawTile.Darw_Sample_BackGroundBox(atvaBackCanvas, back, atvScrData);
        }
    }) as HTMLCanvasElement;
    clsDrawTile.Darw_Sample_BackGroundBox(atvaBackCanvas, atva.Back, atvScrData);
    ...
    const ConvScaleValue = Number(Generic.Convert_ScaleUnit(enmScaleUnit.kilometer, atv.MapScale.Unit));
    ...
    const setSelectValue = cboSobyouAutoDegree.setSelectValue as ((value: string) => void) | undefined;
    setSelectValue?.(String(atv.SouByou.AutoDegree));
    ...
    /**背景・描画●●●●●●●●●●●●●●●●●●●●● */
    const screenMargin = (atv.ScrData as {
        Screen_Margin: {
            rect: { top: number; bottom: number; right: number; left: number };
            ClipF: boolean;
        };
    }).Screen_Margin;
    const tab10 = Generic.createNewFrame(tab.panel[1], "", "", 15, 10, 260, 80, "ウインドウ内余白");
    Generic.createNewWordNumberInput(tab10, "上余白", "%", screenMargin.rect.top, "", 10, 15,undefined, 40, function (obj: HTMLInputElement, v: number) { screenMargin.rect.top = v; }, "")
    Generic.createNewWordNumberInput(tab10, "下余白", "%", screenMargin.rect.bottom, "", 10, 45,undefined, 40, function (obj: HTMLInputElement, v: number) { screenMargin.rect.bottom = v; }, "")
    Generic.createNewWordNumberInput(tab10, "右余白", "%", screenMargin.rect.right, "", 130, 15,undefined, 40, function (obj: HTMLInputElement, v: number) { screenMargin.rect.right = v; }, "")
    Generic.createNewWordNumberInput(tab10, "左余白", "%", screenMargin.rect.left, "", 130, 45,undefined, 40, function (obj: HTMLInputElement, v: number) { screenMargin.rect.left = v; }, "")

    const tab11 = Generic.createNewFrame(tab.panel[1], "", "", 15, 110, 260, 140, "枠・色");
    Generic.createNewCheckBox(tab11, "余白で地図画像クリップ", "", screenMargin.ClipF, 10, 15,undefined,  function (obj: HTMLInputElement) { screenMargin.ClipF = obj.checked }, "");
    ...
    const baseBackCanvas = Generic.createNewWordDivCanvas(tabLegend.panel[0], "BaseBack", "凡例背景", 30, 80,undefined, function(e: MouseEvent){
        ...
        function backGet(back: BackGround_Box_Property) {
            ...
            clsDrawTile.Darw_Sample_BackGroundBox(baseBackCanvas, back, atvScrData);
        }
    }) as HTMLCanvasElement;
    clsDrawTile.Darw_Sample_BackGroundBox(baseBackCanvas, atv.MapLegend.Base.Back, atvScrData);
    ...
    for (let i = 0; i < lLK.length; i++) {
        const lineVisibleStr = String(atvl.Line_Visible_Number_STR);
        const cv = (lineVisibleStr.substring(i, i + 1) === "1");
    ...
    Generic.createNewCheckListBox(tabLegend.panel[3], "", lLKList, 20, 45, 150, 150, function (num: number, checked: boolean) {
        const current = String(atvl.Line_Visible_Number_STR);
        atvl.Line_Visible_Number_STR = current.substring(0, num) + ((checked === true) ? "1" : "0") + current.substring(num + 1);
    }, "");
    const MapLegendLine_DummyKind = Generic.createNewWordDivCanvas(tabLegend.panel[3], "", "背景", 190, 45, undefined, function (e: MouseEvent) {
        ...
        function backGet(back: BackGround_Box_Property) {
            atvl.Back = back;
            clsDrawTile.Darw_Sample_BackGroundBox(MapLegendLine_DummyKind, back, atvScrData);
        }
    }) as HTMLCanvasElement;
    clsDrawTile.Darw_Sample_BackGroundBox(MapLegendLine_DummyKind, atvl.Back, atvScrData);
    ...
    const atvsBack = Generic.createNewWordDivCanvas(tab.panel[4], "", "背景", 30, 90, undefined, function (e: MouseEvent) {
        ...
        function backGet(back: BackGround_Box_Property) {
            atvs.Back = back;
            clsDrawTile.Darw_Sample_BackGroundBox(atvsBack, back, atvScrData);
        }
    }) as HTMLCanvasElement;
    clsDrawTile.Darw_Sample_BackGroundBox(atvsBack, atvs.Back, atvScrData);
        ...
        switch (tab.selectedIndex) {
            case 0: {
                ...
                for (let i = 0; i < checked.length; i++) {
                    ...
                    const soloModeRaw: unknown = Generic.getRadioCheckByValue("soloModeSelect");
                    (dt as unknown as { SoloMode: number }).SoloMode = Number(soloModeRaw);
                    if (appState().attrData.Check_Enable_SoloMode(dt.SoloMode, LayerNum, n) === true) {
                        DataItem.push(dt);
                    } else {
                        emes += appState().attrData.Get_DataTitle(LayerNum, n, false) + "は" + String(Generic.getSolomodeStrings(dt.SoloMode)) + "モードで表示できません。" + "\n";
                    }
                }
                break;
            }
...
export function frmMain_MarkPosition(okEvent: (mode: number) => void) {
    ...
    const cboLonData = Generic.createNewWordSelect(gbDataSelect, "経度", [], 0, "", 10, 20, 30, 140, 0, undefined, "", "", true) as SelectControl;
    const cboLatData = Generic.createNewWordSelect(gbDataSelect, "緯度", [], 0, "", 10, 50, 30, 140, 0, undefined, "", "", true) as SelectControl;
    ...
    function buttonOK() {
        const mode = Number(Generic.getRadioCheckByValue("markPositionOperation"));
        switch (mode) {
...
export function frmMain_ConditionSettings(okEvent: () => void){
    ...
    const conditions = appState().attrData.TotalData.Condition as Array<{
        Name: string;
        Enabled: boolean;
        Layer: number;
        Condition_Class: strCondition_Data_Info[];
    }>;

    Generic.createNewButton(backDiv,"追加","",110,275,function(){
        const cttl=[];
        for(let  i = 0 ;i< conditions.length;i++){
            cttl[i] = String(conditions[i].Name);
        }
        ...
        const conditionClasses = ConItem.Condition_Class as strCondition_Data_Info[];
        conditionClasses.push(cdt);
...
function frmMain_ConditionSettingSub(_ConItem: strCondition_DataSet_Info, okEvent: (ConItem: strCondition_DataSet_Info) => void) {
    ...
    const sourceConItem = _ConItem as strCondition_DataSet_Info & { Clone?: () => unknown };
    const ConItem = (typeof sourceConItem.Clone === 'function' ? sourceConItem.Clone() : sourceConItem) as strCondition_DataSet_Info & {
        Layer: number;
        Name: string;
        Enabled: boolean;
        Condition_Class: strCondition_Data_Info[];
    ...
    appState().attrData.Set_LayerName_to(cboLayerLabel, LayerNum);
    const txtTitle = Generic.createNewWordTextInput(grTop, "タイトル", "", ConItem.Name, "", 15, 50, 60, 160, undefined, "text-align:left") as HTMLInputElement;

    const cboStep = Generic.createNewWordSelect(backDiv, "段階", undefined, -1, "", 15, 140, 40, 80, 0, changeStep, "", "") as SelectControl;
    Generic.createNewButton(backDiv, "段階追加", "", 170, 140, function () {
        const n = cboStep.options.length;
        const cdt = new strCondition_Data_Info();
        cdt.And_OR = enmConditionAnd_Or._And;
        cdt.Condition = [];
        const classes = ConItem.Condition_Class as strCondition_Data_Info[];
        classes.push(cdt);
        setStep(n);
    }, "");
    ...
    const txtValue = Generic.createNewWordTextInput(newSettings, "条件値", "", "", "", 15, 45, 60, 150, undefined, "text-align:left") as HTMLInputElement;
    const unitSpan = Generic.createNewSpan(newSettings, "", "", "", 235, 45, "", undefined)
    // 初期化処理
    ConItem.Layer = Number(LayerNum);
    ...
    Generic.createNewButton(newSettings, "項目追加", "", 300, 75, function (e: MouseEvent) {
        ...
        const Lim = new strCondition_Limitation_Info();
            Lim.Data = Number(cboData.selectedIndex);
...
export function frmMain_AreaPeripheri(okEvent: () => void){
    ...
    function buttonOK(/*e: MouseEvent*/) {
        ...
        const Data_Val_STR: (string | number | undefined)[] = new Array<string | number | undefined>(n);
        let Title = "";
        const note = "";
        const MisF = false;
        const ScaleUnit = Number(scaleUnit.getValue());
        const ScaleRatio = Number(Generic.Convert_ScaleUnit(enmScaleUnit.kilometer, ScaleUnit));
        const SUnit = Generic.getScaleUnitStrings(undefined,ScaleUnit);
        let Unit = "";
        const rv = Number(Generic.getRadioCheckByValue("rbAreaOrPeri") as unknown as number);
        switch (rv) {
            case 0: {
                for (let i = 0; i < n; i++) {
                ...
                Title = "計測面積";
                    Unit = String(Generic.getScaleUnitAreaStrings(ScaleUnit));
                break;
            }
            case 1: {
                for (let i = 0; i < n; i++) {
...
export function frmMain_Culc(okEvent: () => void) {
    ...
    const cboEndData = Generic.createNewWordSelect(backDiv, "期末データ", [], -1, "", 100, 385, 60, 150, 0, function (/*e: Event*/) { Generic.checkRadioByValue("dataCulMethod", 4) }, "", "") as SelectControl;
    ...
    const cboDensityData = Generic.createNewSelect(backDiv, [], -1, "", 100, 415, false, function (/*e: Event*/) { Generic.checkRadioByValue("dataCulMethod", 5) }, "width:150px") as SelectControl;//密度
    ...
    function buttonOK(/*e: MouseEvent*/) {
        ...
        const mode = Number(Generic.getRadioCheckByValue("dataCulMethod") as unknown as number);
        switch (mode) {
            case 0: {
...
export function frmMain_GetDistance(okEvent: () => void){
    ...
    function (/*e: MouseEvent*/) {
        frmLatLonInput(new latlon(0,0), true, function (lp: latlon) {
            const s = Generic.Get_LatLon_Strings(lp, true) as { x: string; y: string };
            const tx=s.x + "/" + s.y;
    ...
    function buttonOK(e: MouseEvent) {
                ...
                switch (pos[i].type) {
                    case DisType.Point: {
                        const P = spatial.Get_Converted_XY(pos[i].xy, appState().attrData.TotalData.ViewStyle.Zahyo);
                            d = Number(appState().attrData.Distance_Kencode_Point(LayerNum, j, P));
                        break;
                    }
                    case DisType.LayerObject: {
                            d = Number(appState().attrData.Distance_Kencode_Object(j, pos[i].objpos, LayerNum, pos[i].lay));
                        break;
                    }
                    case DisType.LayerDummy: {
                        const L = appState().attrData.LayerData[LayerNum];
                        const mapFileData = L.MapFileData as unknown as {
                            Distance_ObjectCenterP: (...args: unknown[]) => unknown;
                            Distance_Object: (...args: unknown[]) => unknown;
                        };
                        if((L.Type===enmLayerType.DefPoint)||(L.Type===enmLayerType.Mesh)){
                            d = Number(mapFileData.Distance_ObjectCenterP(L.atrObject.atrObjectData[j].CenterPoint, pos[i].objpos, L.Time));
                        }else{
                            d = Number(mapFileData.Distance_Object(appState().attrData.Get_KenObjCode(LayerNum, j), pos[i].objpos, L.Time, L.Time));
                        }
                        break;
                    }
                    case DisType.MapObject: {
                            d = Number(appState().attrData.Distance_Kencode_MPObject(LayerNum, j, pos[i].MapFile, pos[i].objpos, pos[i].Time));
                        break;
                    }
                }
                dis[i][j] = d;
        ...
        const Unit = Number(scaleUnit.getValue());
        const ScaleRatio = Number(Generic.Convert_ScaleUnit(enmScaleUnit.kilometer, Unit));
        const lbText = lbList.getAllText();
        const rv = Number(Generic.getRadioCheckByValue("rbDisGet") as unknown as number);
        switch (rv) {
            case 0: {
                //最小距離取得
                let tx = "最も近い距離";
...
export function frmMain_LayeObjectSelectOne(Dummy_Select_EnableF: boolean,DefLayerNum: number, DefSelectObjectNumber: number,defDummyF: boolean, okEvent: (LayerNum: number, SelectObjcetNo: number, DummySelectF: boolean) => void) {
    ...
    function buttonOK() {
        ...
        const selObj = Number(lstObject.selectedIndex);
        if(selObj===-1){
...
function frmTitleSettingsAddingData(TTL: string, UNT: string, Note: string, CancefFlag: boolean, LabelStr: string, okEvent: (retV: {title: string, unit: string, note: string}) => void) {
    ...
    const ttlBox = Generic.createNewWordTextInput(backDiv, "タイトル", "", TTL, "", 15, 70, 50, 250, undefined, "") as HTMLInputElement;
    const unitBox = Generic.createNewWordTextInput(backDiv, "単位", "", UNT, "", 15, 100, 50, 250, undefined, "") as HTMLInputElement;
    const noteBox = Generic.createNewWordTextInput(backDiv, "注", "", Note, "", 15, 130, 50, 250, undefined, "") as HTMLInputElement;
...
export function frmMain_Buffer(okEvent: (e: MouseEvent) => void){
    ...
    const cboHandledLayer = Generic.createNewWordSelect(gbLayerMethod, "検索対象レイヤ", undefined, -1, "", 10, 20, 100, 150, 1, setDataList, "", "") as SelectControl;
    ...
    function setDataList(){
        const L = Number(cboHandledLayer.selectedIndex);
        const n = Number(Generic.getRadioCheckByValue("rbSearchMethod") as unknown as number);
    ...
    function checkError(e: MouseEvent) {
        let ef = false;
        let efm = "";
        const L2 = Number(cboHandledLayer.selectedIndex);
        const rd = Number(Generic.getRadioCheckByValue("rbSearchMethod") as unknown as number);
    ...
    function Buffering(){
        ...
        const BufferMode = Number(Generic.getRadioCheckByValue("rbSearchMethod") as unknown as number);
        let F_ObjectCount; 
        const ObjCount_STR=[];
        const ObjName_STR=[];
        ...
        //----main
        const bufUnit  = Number(scaleUnit.getValue());
        const ScaleRatio  = Number(Generic.Convert_ScaleUnit(enmScaleUnit.kilometer, bufUnit));
        const Dis  = Number(txtBuffer.value);
...
export function openMapFile(call: (data: JsonValue, filename?: string) => void) {
    ...
    const getFile = function (file: File) {   
        ...
        function unzipError(/*err: Error*/) {
            const rs = (globalThis as { readingScreen?: { close?: () => void } }).readingScreen;
            rs?.close?.();
            //Generic.clear_backDiv();
            Generic.clear_backDiv();
            call(undefined);
    ...
    //ファイル選択する場合
    const fileIn = Generic.createNewInput(bbox, "file", "", "", 15, 75, "", "") as HTMLInputElement;
    fileIn.accept=".mpfj";
    ...
    dropZone.addEventListener('drop', function (e: DragEvent) {
        ...
        const file = files[0];
        if (String(Generic.getExtension(file.name)).toLowerCase() !== "mpfj") {
...
export function frmCopyObjectName(MapData: IMapData, initParapeter: strFrmCopyObjectName_init_parameter_data, okEvent: (copyData: string) => void, cancelEvent: (() => void) | undefined = undefined) {
    ...
    const objNameBox = Generic.createNewInput(backDiv, "text", initParapeter.ObjName, "", 20, 60, undefined, "width:160px") as HTMLInputElement;
    ...
    const objectKinds = MapData.ObjectKind as CopyObjectKindInfo[];
    for (let i = 0; i < MapData.Map.OBKNum; i++) {
        const objectKind = objectKinds[i];
        ...
        for (let i = 0; i < tx0.length; i++) {
            switch (tx0.charCodeAt(i)) {
                ...
                case 8:
                    break;
                default:
                    tx += tx0.substring(i, i + 1);
                    break;
            }
        }
        objNameBox.value = tx;
        const retComp = Generic.ObjName_Kanji_Compatible(tx) as { newObjname: string };
        tx = String(retComp.newObjname);
...
export function frmPrint_DummyObjectGroup(){
    ...
    const selLayer = Generic.createNewWordSelect(gbDummyFrame,"レイヤ", undefined, -1, "", 10, 30,50,150,0,  setDummyObjectList,"", "") as SelectControl;
    ...
    const txtDummy = Generic.createNewInput(gbDummyList, "text", "", "", 10, 185, undefined, "width:100px") as HTMLInputElement;
    ...
    Generic.createNewButton(gbDummyList, "オブジェクト名コピーパネル", "", 10, 210, function (/*e: MouseEvent*/) {
        ...
        const objectGroupChecked = init_para.ObjectGroupChecked as boolean[];
        const obkNum = Number(mp.Map.OBKNum);
        objectGroupChecked.length = obkNum;
        objectGroupChecked.fill(true);
    ...
    function showPointMark() {
        ...
        for (let i = 0; i < DOPnum; i++) {
            ...
            const y = i * lineHeight + 3;
                const lc = Generic.createNewWordDivCanvas(pnlPointMarkList, "", lk.ObjectKindName, 10, y, 100, inePatternClick) as HTMLElement & { tag: number };
            lc.tag = i;
    ...
    function setDummyObjectList() {
        LayerNum = Number(selLayer.selectedIndex);
