...
interface strDegreeMinuteSeconde {
    Degree: number;
    Minute: number;
    Second: number;
}
...
interface strLatLonDegreeMinuteSecond {
...
Start_End_Time_data.prototype.Clone=function(this: Start_End_Time_data) {
    let d=new Start_End_Time_data();
    d.StartTime=this.StartTime.Clone();
    d.EndTime=this.EndTime.Clone();
    return d;
}
Start_End_Time_data.prototype.Equals=function(this: Start_End_Time_data, SETime: Start_End_Time_data) {
...
//属性データのタイプ
// var enmAttDataType = { // globals.d.tsで定義済み
//     Normal: 0,
//     Category: 1,
//     Strings: 2,
//     URL: 3,
...
var strObject_Data_Info: any = function (this: strObject_Data_Info) {
    ...
    this.MeshRect=new rectangle(0, 0, 0, 0);
...
interface strObject_Info {
    ObjectNum: number;
    NumOfSyntheticObj: number;
...
//合成オブジェクト名とコード（属性データ）
interface strSynthetic_ObjectName_and_Code {
    code: number;
    Name: string;
    Draw_F: boolean;
...
//合成オブジェクト（属性・地図データ）
interface strSynthetic_Object_Data {
    Kind: number;
    NumOfObject: number;
    Name: string;
    CenterP: point;
...
/**タイルマップを描画する際の情報 */
interface strTileMapViewInfo {
    Visible: boolean;
    AlphaValue: number;// 0-1
    TileMapDataSet: any;// tileMapList.getTileInfo
...
var strTileMapViewInfo: any = function (this: strTileMapViewInfo) {
    ...
    this.DrawTiming = 0;//enmDrawTiming
}
...
interface tileList_Data_Info {
    LatLonBox: any;// strLatLonBox (未定義)
...
var strClassPaint_Data: any = function (this: strClassPaint_Data) {
    ...
    this.Color_Mode = 0;//enmPaintColorSettingModeInfo
}
...
interface strMarkSizeModeLineShapeData {
    LineWidth: number;  //Single
...
//記号の大きさモード設定（属性データ）
interface strMarkSize_Data {
    MaxValueMode: number;  //enmMarkSizeValueMode
    MaxValue: number;  //Double
    Value: number[];  //Double '4
    Mark: Mark_Property;
...
//記号の数モード設定（属性データ）
interface strMarkBlock_Data {
    Value: number;// Double
    ArrangeB: number;// enmMarkBlockArrange
    HasuVisible: boolean;// Boolean
    Mark: Mark_Property;
    Overlap: number;// Integer
...
interface strMarkBar_Data {
    Width: number;  //Single
    MaxHeight: number;  //Single
    MaxValueMode: number;  //enmMarkSizeValueMode
    MaxValue: number;  //Double
    InnerTile: Tile_Property;
    FrameLinePat: Line_Property;
    ThreeD: boolean;  //Boolean
    ScaleLineInterval: number;  //Double
    ScaleLineVisible: boolean;  //Boolean
...
var strMarkBar_Data: any = function (this: strMarkBar_Data) {
    ...
    this.ThreeD = false;  //Boolean
    this.ScaleLineInterval = 0;  //Double
    this.ScaleLineVisible = false;  //Boolean
    this.scaleLinePat = new Line_Property();
    this.BarShape = 0;//enmMarkBarShape
}
...
//文字モード
interface strString_Data {
    Font: Font_Property;
    maxWidth: number;  //Single
...
//階級区分データ（属性データ）
interface strClass_Div_data {
    Value: number | string;  //Double/String
...
var strContour_Data_Regular_interval: any = function (this: strContour_Data_Regular_interval) {
    ...
    this.SP_Bottom = 0; // Double
    this.SP_interval = 0; // Double
    this.SP_Top = 0; // Double
    this.SP_Line_Pat = new Line_Property();
    this.EX_Value_Flag = false; // Boolean
    this.EX_Value = 0; // Double
    this.EX_Line_Pat = new Line_Property();
}
interface strContour_Data_Irregular_interval {
    Value: number; // Double
    Line_Pat: Line_Property;
...
interface strContour_Data {
    Interval_Mode: number; // enmContourIntervalMode
    Draw_in_Polygon_F: boolean; // Boolean
    Spline_flag: boolean; // Boolean
    Detailed: number; // Integer
    Regular: strContour_Data_Regular_interval;
    IrregularNum: number; // Integer
...
var strContour_Data: any = function (this: strContour_Data) {
    ...
    this.Detailed = 0; // Integer
    this.Regular = new strContour_Data_Regular_interval();
    this.IrregularNum = 0; // Integer
...
interface strContour_Line_property {
    Flag: boolean; // Boolean
    Layernum: number; // Integer
    DataNum: number; // Integer
    Value: number; // Double
    NumOfPoint: number; // Integer
...
var strContour_Line_property: any = function (this: strContour_Line_property) {
    ...
    this.Value = 0; // Double
    this.NumOfPoint = 0; // Integer
    this.PointStac = 0; // Integer
    this.Circumscribed_Rectangle = new rectangle();
}
//線引きモード（属性データ）
interface strClassODMode_data {
    o_Layer: number; // Integer
    O_object: number; // Integer
    Dummy_ObjectFlag: boolean; // Boolean
    Arrow: Arrow_Data;
...
var strStatisticInfo: any = function (this: strStatisticInfo) {
    ...
    this.Sum = 0; //Double
    this.sa = 0; //Double
    this.BeforeDecimalNum = 0; //Integer
    this.AfterDecimalNum = 0; //Integer
}
...
//データ項目の情報
interface strData_info {
    Title: string;  //String
    Unit: string;  //String
    MissingF: boolean;  //Boolean
    Note: string;  //String
    DataType: number;  //enmAttDataType
    MissingValueNum: number;  //Integer
    EnableValueNum: number;  //Integer
    Statistics: strStatisticInfo;
    ModeData: number;  //enmSoloMode_Number
    SoloModeViewSettings: strSoloModeViewSettings_Data;
    Value: string[];  //String
...
var strData_info: any = function (this: strData_info) {
    ...
    this.Note = "";  //String
    this.DataType = 0;  //enmAttDataType
    // 欠損値のデータ数
    this.MissingValueNum = 0;  //Integer
    // 欠損値を除いたデータ項目中のデータ数
    this.EnableValueNum = 0;  //Integer
    this.Statistics = new strStatisticInfo();
    //  表示モード
    this.ModeData = 0;  //enmSoloMode_Number
...
strData_info.prototype.Clone = function (NoValueFlag=false) {
    ...
    let dts=dt.SoloModeViewSettings;
...
interface strShowViewerLayerInfo {
    Name: string; //String
...
interface strLayerPointLineShape_Data {
    LineWidth: number; //Single
    LineEdge: LineEdge_Connect_Pattern_Data_Info;
...
/**ラベルモード全体 */
interface strLabelMode_Data_info {
    SelectedIndex: number;// Integer
    DataSet: strLabel_Data[];
    initDataSet(): void;
    AddDataSet(): void;
...
/**ラベルモードの個別データセットの構造体*/
interface strLabel_Data {
    title: string; //String
    Location_Mark_Flag: boolean; //Boolean
    Location_Mark: Mark_Property;
    Width: number; //Single
    DataItem: number[]; //Integer
    DataValue_Font: Font_Property;
    DataValue_Unit_Flag: boolean; //Boolean
    DataValue_TurnFlag: boolean; //Boolean
    DataValue_Print_Flag: boolean; //Boolean
    DataName_Print_Flag: boolean; //Boolean
...
var strLabel_Data: any = function (this: strLabel_Data) {
        ...
        this.DataValue_Unit_Flag = false; //Boolean
        this.DataValue_TurnFlag = false; //Boolean
        this.DataValue_Print_Flag = false; //Boolean
        this.DataName_Print_Flag = false; //Boolean
...
//グラフモード全体の構造体
interface strGraphMode_DataSetting_Info {
    SelectedIndex: number;// Integer
    DataSet: strGraph_Data[];
    initDataSet(): void;
    AddDataSet(): void;
...
/**グラフモード個別データセット */
interface strGraph_Data {
    title: string; //String
    GraphMode: number; //enmGraphMode
    Data: GraphModeDataItem[];
    En_Obi: strGraph_Data_En;
    Oresen_Bou: strGraph_Data_Oresen;
    initData(): void;
    Clone(): strGraph_Data;
}
...
interface GraphModeDataItem {
    DataNumber: number;// Integer
    Tile: Tile_Property;
...
interface strGraph_Data_En {
    EnSizeMode: number; //enmGraphMaxSize
    EnSize: number; //Single
    Value1: number; //Double
    Value2: number; //Double
    Value3: number; //Double
    BoaderLine: Line_Property;
    AspectRatio: number; //Single
    StackedBarDirection: number; //enmStackedBarChart_Direction
    RMAX: number; //Double
    RMIN: number; //Double
    MaxValueMode: number; //enmMarkMaxValueType
    MaxValue: number; //Double
...
var strGraph_Data_En: any = function (this: strGraph_Data_En) {
    ...
    this.Value2 = 0; //Double
    this.Value3 = 0; //Double
    this.BoaderLine=new Line_Property();
    this.AspectRatio = 0; //Single
    this.StackedBarDirection = 0; //enmStackedBarChart_Direction
    this.RMAX = 0; //Double
    this.RMIN = 0; //Double
    this.MaxValueMode = 0; //enmMarkMaxValueType
    this.MaxValue = 0; //Double
}
...
interface strGraph_Data_Oresen {
    Size: number; //Single
    Line: Line_Property;
    AspectRatio: number; //Single
    YmaxMinMode: number; //enmBarLineMaxMinMode
    YMax: number; //Double
    Ymin: number; //Double
    Ystep: number; //Double
...
var strGraph_Data_Oresen: any = function (this: strGraph_Data_Oresen) { //複数表示（折れ線）（属性データ）
    ...
    this.AspectRatio = 0; //Single
    this.YmaxMinMode = 0; //enmBarLineMaxMinMode
    this.YMax = 0; //Double
    this.Ymin = 0; //Double
    this.Ystep = 0; //Double
    this.BackgroundTile=new Tile_Property();
    this.BorderLine=new Line_Property();
    this.FrameAxe = 0;//enmBarChartFrameAxePattern
}
...
interface strLayerModeViewSetting_Data {
   LabelMode: strLabelMode_Data_info;
   GraphMode: strGraphMode_DataSetting_Info;
...
var strLayerModeViewSetting_Data: any = function (this: strLayerModeViewSetting_Data) {
   ...
   this.GraphMode = new strGraphMode_DataSetting_Info();
    ...
    this.PolygonDummy_ClipSet_F = false; //Boolean
}

/**ダミーオブジェクトの配列 */
interface strDummyObjectName_and_Code {
    code: number; //Integer
    Name: string; //String
...
interface ODBezier_Data {
    ObjectPos: number;//Integer
    Data: number;//Integer
    Point: point;
    Clone(): ODBezier_Data;
}
...
interface strLayerDataInfo {
    ...
    TripTimeSpan: any;// = new TripTimeSpan_Info;
    TripType: number;//enmTripPositionType
    atrObject: strObject_Info;
    atrData: stratrData_Info;
    Dummy: strDummyObjectName_and_Code[];// new strDummyObjectName_and_Code();
    DummyGroup: any[];// 番号;
    Print_Mode_Layer: number;//enmLayerMode_Number '0:単独 1:グラフ 2:ラベル 3:移動
    LayerModeViewSettings: strLayerModeViewSetting_Data;
...
var strLayerDataInfo: any = function (this: strLayerDataInfo) {
    ...
    this.Name = "";//String
    this.MapFileName = "";//String
    this.MapFileData = undefined;//clsMapData
    this.MapFileObjectNameSearch = undefined;//clsObjectNameSearch
    this.Shape = 0;//enmShape
    this.Type = 0;//enmLayerType
    this.MeshType = 0;//enmMesh_Number
    this.ReferenceSystem = 0;//enmZahyo_System_Info
    this.Time = new strYMD();
    this.Comment = "";//String
    this.TripTimeSpan = undefined;// = new TripTimeSpan_Info;
    this.TripType = 0;//enmTripPositionType
    // オブジェクトの情報
    ...
    this.DummyGroup=[];// 番号;
    this.Print_Mode_Layer = 0;//enmLayerMode_Number '0:単独 1:グラフ 2:ラベル 3:移動
    // グラフ表示、ラベル表示、移動表示、点線オブジェクトのペイントモードの記号
    this.LayerModeViewSettings = new strLayerModeViewSetting_Data();

    this.PrtSpatialIndex = undefined;//clsSpatialIndexSearch
...
strLayerDataInfo.prototype.Get_OD_Bezier_RefPoint = function (ObjPos: number, DataNum: number) {
...
/**線モードのベジェ曲線用の参照地点を削除 */
strLayerDataInfo.prototype.Remove_OD_Bezier = function (ObjPos: number, DataNum: number) {
    for (let i = 0; i < this.ODBezier_DataStac.length; i++) {
        let bs = this.ODBezier_DataStac[i];
        if ((bs.Data == DataNum) && (bs.ObjectPos == ObjPos)){
...
strLayerDataInfo.prototype.Add_OD_Bezier = function (ObjPos: number, DataNum: number, RefPoint: point) {
    for (let i = 0; i < this.ODBezier_DataStac.length; i++) {
        let bs = this.ODBezier_DataStac[i];
        if ((bs.Data == DataNum) && (bs.ObjectPos == ObjPos)){
...
var strScreen_Setting_Data_Info: any = function (this: strScreen_Setting_Data_Info) {
    ...
    this.Accessory_Base = 0;//enmBasePosition
...
interface strOverLay_DataSet_Item_Info {
    Layer: number;
    DataNumber: number;
    Print_Mode_Layer: number;//enmLayerMode_Number
    Mode: number;//enmSoloMode_Number
...
var strOverLay_DataSet_Item_Info: any = function (this: strOverLay_DataSet_Item_Info) {
    ...
    this.Mode = 0;//enmSoloMode_Number
    this.Legend_Print_Flag = false;//Boolean
...
// </summary>
// <remarks></remarks>
interface strOverLay_Dataset_Info {
    title: string;
...
// 重ね合わせモード全体のデータ
interface strOverLayMOde_Dataset_Info {
    SelectedIndex: number;
...
interface strSeries_DataSet_Item_Info {
    Layer: number;
    Data: number;
    Print_Mode_Total: number;// enmTotalMode_Number
    Print_Mode_Layer: number;// enmLayerMode_Number
...
var strSeries_DataSet_Item_Info: any = function (this: strSeries_DataSet_Item_Info) {//連続表示モード
    ...
    this.Print_Mode_Layer = 0;// enmLayerMode_Number
    this.SoloMode = 0;// enmSoloMode_Number
}
...
interface strSeries_Dataset_Info {
    title: string;
    SelectedIndex: number;
...
var strSeries_Dataset_Info: any = function (this: strSeries_Dataset_Info) {
    ...
    this.AddData = function (LayerIndex: number, DataIndex: number, TotalDataViewMode: number, LayerDataVieMode: number, SoloViewMode: number) {
        let d = new strSeries_DataSet_Item_Info()
        d.Layer = LayerIndex;
        d.Data = DataIndex;
...
// 連続表示モード全体の構造体
interface strSeriesMode_Dataset_Info {
    SelectedIndex: number;
...
//欠損値の設定（属性データ）
interface strMissing_set {
    Print_Flag: boolean; //Boolean
    Text: string; //String
...
//属性データ基本値（属性データ）
interface strBasic_Data {
    Lay_Maxn: number; //Integer
    SelectedLayer: number; //Integer
    Print_Mode_Total: number; //enmTotalMode_Number '0:データ表示 1:重ね合わせ　2:連続
    Comment: string; //String
    MDRFileVersion: number; //Single
...
var strBasic_Data: any = function (this: strBasic_Data) {
    ...
    this.Print_Mode_Total = 0; //enmTotalMode_Number '0:データ表示 1:重ね合わせ　2:連続
    this.Comment = ""; //String
    this.MDRFileVersion = 0; //Single
    this.FileName = ""; //String
    this.FullPath = ""; //String
    this.DataSourceType = 0; //enmDataSource
}
...
//     _Or :1
// }
interface strCondition_Limitation_Info {
    Data: number;// Integer
    Condition: number;// enmCondition
    Val: string; // String
...
interface strCondition_Data_Info {
    And_OR: number;//enmConditionAnd_Or
...
interface strCondition_DataSet_Info {
...
interface Total_Data_Info {
    LV1: strBasic_Data;
    TotalMode: strTotalMode_Info;
    ViewStyle: strViewStyle_Info;
    FigureStac: any[];
...
interface strDummyObjectPointMark_Info {
...
interface strLegend_Base_Attri {
    Visible: boolean; //Boolean
    Legend_Num: number; //Integer
    Font: any; //Font_Property (未定義)
    Back: any; //BackGround_Box_Property (未定義)
    LegendXY: point[]; // PointF
    Comma_f: boolean; //Boolean
...
var strLegend_Base_Attri: any = function (this: strLegend_Base_Attri) {
    ...
    this.Back = new BackGround_Box_Property(); //BackGround_Box_Property
    this.LegendXY = []; // As PointF
    this.Comma_f = false; //Boolean
    this.ModeValueInScreenFlag = false;
}
...
interface strClassBoundaryLine_Info {
    Visible: boolean;
    LPat: Line_Property;
...
interface strLegend_Class_Attri {
    PaintMode_Line: Line_Property;
    PaintMode_Method: number; //enmClassMode_Meshod
    CategorySeparate_f: boolean; //Boolean
    PaintMode_Width: number; //Single
    ClassMarkFrame_Visible: boolean; //Boolean
    SeparateClassWords: number; //enmSeparateClassWords
    SeparateGapSize: number; //Single
    ClassBoundaryLine: strClassBoundaryLine_Info;
...
var strLegend_Class_Attri: any = function (this: strLegend_Class_Attri) {
    ...
    this.PaintMode_Width = 0; //Single
    this.ClassMarkFrame_Visible = false; //Boolean
    this.SeparateClassWords = 0; //enmSeparateClassWords
    this.SeparateGapSize = 0; //Single
    this.ClassBoundaryLine = new strClassBoundaryLine_Info();
    this.FrequencyPrint = false; //Boolean
}
...
interface strLegend_Mark_Attri {
    CircleMD_CircleMini_F: boolean; //Boolean
...
var strLegend_Line_Dummy_Attri: any = function (this: strLegend_Line_Dummy_Attri) {
    ...
    this.Dummy_Point_Visible = false; //Boolean
...
interface strOverLay_Legend_Title_Info {
    Print_f: boolean;
    MaxWidth: number;
...
interface strLegend_Attri {
    Base: strLegend_Base_Attri;
    OverLay_Legend_Title: strOverLay_Legend_Title_Info;
    ClassMD: strLegend_Class_Attri;
    MarkMD: strLegend_Mark_Attri;
    Line_DummyKind: strLegend_Line_Dummy_Attri;
...
var strLegend_Attri: any = function (this: strLegend_Attri) {
    ...
    this.En_Graph_Pattern = 0; //enmMultiEnGraphPattern
}
...
interface strLatLonLine_Print_Info {
    Visible: boolean;
    Order: number;//enmLatLonLine_Order
    Lat_Interval: number;
    Lon_Interval: number;
    LPat: Line_Property;
...
interface strAccessoryGroupBox_Info {
    Visible: boolean;  // Boolean
    Back: any;//BackGround_Box_Property (未定義)
    Title: boolean;  // Boolean
    Comapss: boolean;  // Boolean
    Scale: boolean;  // Boolean
    Legend: boolean;  // Boolean
    Note: boolean;  // Boolean
    LinePattern: boolean;  // Boolean
...
var strAccessoryGroupBox_Info: any = function (this: strAccessoryGroupBox_Info) {
    ...
    this.Comapss = false;  // Boolean
    this.Scale = false;  // Boolean
    this.Legend = false;  // Boolean
    this.Note = false;  // Boolean
    this.LinePattern = false;  // Boolean
    this.ObjectGroup = false;  // Boolean
}
...
interface strValueShow_Info {
    ValueVisible: boolean;//Boolean
    ValueFont: Font_Property;
    DecimalSepaF: boolean;//Boolean
    DecimalNumber: number;
    ObjNameVisible: boolean;//Boolean
...
var strValueShow_Info: any = function (this: strValueShow_Info) {
    ...
    this.ObjNameVisible = false;//Boolean
...
interface strViewStyle_Info {
    ...
    AttMapCompass: any;//strCompass_Attri (未定義)
    MapLegend: strLegend_Attri;
    FigureVisible: boolean; //Boolean
    AccessoryGroupBox: strAccessoryGroupBox_Info;
    ValueShow: strValueShow_Info;
    Missing_Data: strMissing_set;
    Screen_Back: any;//strScreen_Back_data (未定義)
    SymbolLine: strSymbol_Lien_Data;
    Trip_Line: any; //strTrip_Line_Data (未定義)
    PointPaint_Order: number; //enmPointOnjectDrawOrder
    Dummy_Size_Flag: boolean; //Boolean
    MeshLine: Line_Property;
    TileLicenceFont: Font_Property;
    ObjectLimitationF: boolean; //Boolean
    InVisibleObjectBoundaryF: boolean; //Boolean
    DummyObjectPointMark: strDummyObjectPointMark_Info[];
    MapPrint_Flag: boolean; //Boolean
    LatLonLine_Print: strLatLonLine_Print_Info;
    SouByou: any;//strSouByou_Info (未定義)
    TileMapView: strTileMapViewInfo;
...
var strViewStyle_Info: any = function (this: strViewStyle_Info) {
    ...
    this.AttMapCompass = new strCompass_Attri();
    this.MapLegend = new strLegend_Attri();
    this.FigureVisible = false; //Boolean
...
//スケール設定（属性データ）
interface strScale_Attri {
    Visible: boolean; //Boolean
    Position: point;
    Font: Font_Property;
    BarPattern: number; //enmScaleBarPattern
    BarAuto: boolean; //Boolean
    BarDistance: number; //Single
    BarKugiriNum: number; //Integer
    Back: any;//BackGround_Box_Property (未定義)
...
var strScale_Attri: any = function (this: strScale_Attri) {
    ...
    this.Font = new Font_Property();
    this.BarPattern = 0; //enmScaleBarPattern
    this.BarAuto = false; //Boolean
    this.BarDistance = 0; //Single
    this.BarKugiriNum = 0; //Integer
    this.Back = new BackGround_Box_Property();
    this.Unit = 0; //enmScaleUnit
}
...
interface ScreenMargin {
    ClipF: boolean; //Boolean
    rect: rectangle;
    Clone(): ScreenMargin;
}
...
//3Dモードの回転に使用（属性データ）
interface strThreeDMode_Set {
    Set3D_F: boolean; //Boolean
...
interface Screen_info {
    FirstScreenMGMul: number; //Single '全体が表示してある場合の拡大係数(MDRには保存しない)
    GSMul: number; //Double '地図サイズに対するウィンドウサイズの比(MDRには保存しない)
    STDWsize: number; //Single
    ScrView: rectangle;
    ...
    ThreeDMode: strThreeDMode_Set;
    init(pictureboxSize: any, picBoxMargin: any, MapAllAreaRect: any, AccessoryBase: any, SCRViewResetF: any): void;
    Set_PictureBox_and_CulculateMul(pictureboxSize: any): void;
    Get_Screen_BaseMul(): void;
    Get_Length_On_BaseMap(ScreenLength: number): number;
    Get_Length_On_Screen(BaseMapLength: number): number;
    Radius(R_Percent: any, Value: any, max_Value: any): number;
    getSx(x: number): number;
    getSy(y: number): number;
    Get_SxSy_With_3D(x: number, y: number, z: number): point;
    Get_MapDataSize_from_ScreenPixcel(Pixcel: any): void;
    getSXSY_Margin(rec: rectangle): rectangle;
    getSxSy(p: point): point;
    getSxSyRect(rec: rectangle): rectangle;
    getSxSyArray(n: any, XY: any, ReverseGetF: any, SamePointCheck: any): any[];
    getSRX(Sx: number): number;
    getSRY(Sy: number): number;
    getSRXY(SXY: point): point;
    Get_Line_Width(LineWidth: number): number;
    getSxSyfromRatio(RatioP: point): point;
    getRatioPfromSxSy(ScrPos: point): point;
    getRatioPfromSrxSry(SRXY: point): point;
...
var Screen_info: any = function (this: Screen_info) {
    ...
    this.OutputDevide = 0; //enmOutputDevice
    ...
    this.init = function (pictureboxSize: any, picBoxMargin: any, MapAllAreaRect: rectangle, AccessoryBase: number, SCRViewResetF: boolean) {
    ...
    this.Radius = function(R_Percent: number, Value: number, max_Value: number){
        let RR;
        if (max_Value == 0) {
            RR = 0;
        } else {
    ...
    //地図X座標をスクリーン座標に
    this.getSx = function (x: number) {
        let nx = (x - this.ScrView.left) * this.ScreenMG.Mul + this.ScreenMG.Xplus;
        return nx;
    }
    //地図Y座標をスクリーン座標に
    this.getSy = function (y: number) { return (y - this.ScrView.top) * this.ScreenMG.Mul + this.ScreenMG.YPlus }
    ...
    this.getSxSyArray = function (n: number, XY: point[], ReverseGetF: boolean, SamePointCheck: boolean) {
        let j;
        let jp;
        let XY2 = [];
    ...
    //線幅を返す（線幅が0の場合は最小値に）
    this.Get_Line_Width = function (Percentage: number) {
        if (Percentage == 0) {
    ...
    this.getSxSyfromRatio = function(p: point){
        let P2 = new point();
    ...
    this.getSRXYfromRatio = function (p: point) {
        let P2 = new point();
        P2.x = this.ScrRectangle.left + this.ScrRectangle.width() * p.x;
        P2.y = this.ScrRectangle.top + this.ScrRectangle.height() * p.y;
        return P2;
...
interface strObjLocation_and_Data_info {
    ObjLocation: number;
    DataValue: any;
}
...
interface Overlay_Temporaly_Data_Info {
    Printing_Number: number; // Integer
    OverLay_Printing_Flag: boolean; // Boolean  '記号やブロックを重ね合わせる際に標示位置をずらすのに使用
    OverLay_EMode_N: number; // Integer '記号モードをずらす際に使用するが、10では使用しない
...
var Overlay_Temporaly_Data_Info: any = function (this: Overlay_Temporaly_Data_Info) {
    ...
    this.OverLay_EMode_Now = 0; // Integer
    this.Always_Ove_DataStac = []; // List(Of strOverLay_Dat//et_Item_Info)
}

interface Legend2_Atri {
   LineKind_Flag: boolean;// Boolean
   PointObject_Flag: boolean;// Boolean
   Layn: number;// Integer
   DatN: number;// Integer
   Print_Mode_Layer: number;// enmLayerMode_Number
   SoloMode: number;// enmSoloMode_Number
...
interface AccessoryTemp_Infp {
    ...
    Legend_No_Max: number;// Integer
    Push_titleXY: point;
    Push_LegendXY: point;
    Edit_Legend: number;// Integer
...
this.ADD_AttrData= function( InsertData: any ,  AddMapFileNameF: any  ) {
        ...
        if (this.LayerData[Layernum].Type == enmLayerType.Mesh) {
            ...
            switch (this.LayerData[Layernum].atrObject.atrObjectData[ObjNum].Objectstructure) {
                ...
                case enmKenCodeObjectstructure.SyntheticObj: {
                    ...
                    if (f == true) {
                        let ELine = this.Get_Enable_KenCode_MPLine(Layernum, ObjNum, MapP.x)
                        let Fringe_Line = [];
    ...
    this.AddPointObjectKindUsed= function (MapFilename: any,ObjKindNumber: any,MK: any) {
        ...
        let newObk = strObjectKindUsed_Info();
    ...
    this.SetDataFromMDRJ = function (MapDataList: any, attrText: any) {
        ...
        this.TotalData.initTotalData();
        let existFont: string[] = [];
        let nonExistFont: string[] = [];
    ...
    this.Get_EnableMPLine_SyntheticObject = function (Layernum: any, ObjNum: any) {
        ...
        for (let i = 0; i < mp.NumOfObject; i++) {
            if (mp.Objects[i].Draw_F == true) {
                ...
                if (c != -1) {
                    let ELine = LD.MapFileData.Get_EnableMPLine(c, Time);
                    for (let j in ELine) {
    ...
    this.SetIniHanrei = function (Layernum: any, DataNum: any) {
        ...
        if (DType == enmAttDataType.Category) {
            ...
            if (lay.Type != enmLayerType.Trip) {
                ...
                let Max = Math.max(Math.abs(DataMax), Math.abs(DataMin));
                let Min = 0;
                let ST = 0;
