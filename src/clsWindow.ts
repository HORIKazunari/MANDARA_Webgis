...
function setting(locSearch: string) {
    ...
    let firstPanelAbout = AbountInner(state.divMain, 45, 50);

    let divpanel = Generic.createNewDiv(state.divMain, "", "SettingPanel", "", 0o0, state.scrMargin.top, parseInt(state.divMain.style.width), parseInt(state.divMain.style.height), "border-solid 1px;", "");
    ...
    function initFirtScreen() {
        ...
        for(let i in multiMode){
            let ele = doc.getElementById(multiMode[i]);
        ...
        for(let i in complexMode){
            let ele = doc.getElementById(complexMode[i]);
            ele.addEventListener("mouseenter", modeEnter, false);
            ele.addEventListener("mouseleave", modeLeave, false);
    ...
    function setFrequencyLabel() {
        ...
        for (let j = 0; j < div_num; j++) {
            let fbox = doc.getElementById("freqBox" + j);
            if(retv.ok == true) {
    ...
    function pnlGraphEachItem(newRow: number){
        let pnl = doc.getElementById("pnlGraphItem");
        let selGraph = attrData.nowGraph();
        ...
        for (let i = datan; i < pnl.inPanel; i++) {
            let delele = doc.getElementById("pnlGraphIteminPanel" + String(i));
        ...
        for (let i = 0; i < datan; i++) {
            ...
            let ele = doc.getElementById("pnlGraphIteminPanelTextBox" + String(i));
        ...
        function eleClick(){
            ...
            let newSelRow: number = Number(this.tag)
    ...
    function overlayDatasetDataItem(){
        ...
        let overData: (string | undefined)[] = new Array(4);
    ...
    function setSettingSoloModeWindow() {
        ...
        if(attrData.Check_Enable_SoloMode(enmSoloMode_Number.ClassPaintMode, Layernum, DataNum) == true) {
            ...
            switch (layShape) {
                case enmShape.PointShape:
                    ...
                    let ele = doc.getElementById("picPointMarkSize");
            ...
            let div_num = data.SoloModeViewSettings.Div_Num;
            let pnlClassDiv = doc.getElementById("pnlClassDiv");
            ...
            for (let i = div_num; i < pnlClassDiv.inPic; i++) {
                let cbox = doc.getElementById( "picClassBox" + i);
                cbox.parentNode.removeChild(cbox);
                let fbox = doc.getElementById("freqBox" + i);
                fbox.parentNode.removeChild(fbox);
            }
            for (let i = txtNum; i < pnlClassDiv.inTxt; i++) {
                let txele = doc.getElementById( "txtClassValue" + i);
        ...
        if(attrData.Check_Enable_SoloMode(enmSoloMode_Number.MarkSizeMode, Layernum, DataNum) == true) {
            ...
            if(layShape==enmShape.LineShape){
                ...
                let picMark = doc.getElementById("picMarkSize");
        ...
        if(attrData.Check_Enable_SoloMode(enmSoloMode_Number.MarkBlockMode, Layernum, DataNum) == true) {
            ...
            let picMark = doc.getElementById("picMarkBlockSize");
            attrData.Draw_Sample_Mark_Box(picMark, datam.Mark);
    ...
    function SetPictureBox() {
        ...
        for (let i = 0; i < ldd.Div_Num; i++) {
            let p = doc.getElementById("picClassBox" + i) as HTMLCanvasElement;
            if (!p) { continue; }
    ...
    function SetClassDivValueTextBox() {
        ...
        for (let i = 0; i < ldd.Div_Num; i++) {
            if((i != ldd.Div_Num - 1) || (DDType == enmAttDataType.Category)){
                let t = doc.getElementById("txtClassValue" + i);
                let df = false;
...
*/
    ...
    function SetPicClassBoxCursol() {
        ...
        for (let i = 0; i < DivNum; i++) {
            let p = doc.getElementById("picClassBox" + i);
    ...
    function frmSettingMode() {
        ...
        function cboDivisionMethodChange(obj: HTMLElement, sel: number, v: unknown) {
            ...
            let DataNum = attrData.LayerData[Layernum].atrData.SelectedIndex;
            let data = attrData.LayerData[Layernum].atrData.Data[DataNum].SoloModeViewSettings;
            let dc = doc.getElementById("cboDivisionCount");
            let lshape = attrData.LayerData[Layernum].Shape;
            dc.disabled = false;
            switch (v) {
        ...
        Generic.createNewRadioButtonList(gbContourMode, "contourInterval_Mode", contourIntervalList, 10, 10, undefined, 30, undefined,
            function (v: number) {
                ...
                doc.getElementById("gbSeparateSettings").setVisibility(v == enmContourIntervalMode.SeparateSettings);
                Check_Print_err();
            }, "");
    ...
    function Check_Print_err() {
        let errButton = doc.getElementById("settingWindowBtnPrintError");
        let retV = attrData.Get_PrintError();
...
function openShapeFile(okCall: (() => void) | undefined): void{
    ...
    let lst = doc.getElementById("radiozahyoMode2");
    lst.style.top = (lst.offsetTop + 17).px();
    let lsts = doc.getElementById("divradiozahyoMode2");
